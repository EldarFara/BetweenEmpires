...
(team_get_slot, ":global_tactic", "$g_enemy_team", slot_team_pai_global_tactic),
	...
	(try_end),
	(try_begin),
	(eq, ":global_tactic", pai_global_tactic_attack_approaching), # Approaching enemy - searching target that is closest enemy company
	(eq, ":timer", 0),
...
# (display_message, "@distance {reg0}"),
	...
	(position_move_y, pos31, 2500, 0),
		(try_begin),
		(gt, ":distance", 10000), # Approach enemy
		(call_script, "script_team_create_infantry_formation_on_pos31", "$g_enemy_team", formation_type_ranged),
		(try_end),
		(try_begin),
		(le, ":distance", 10000), # Switch to running and use melee formation at 100 meters distance
		(ge, ":distance", 5000),
			(try_for_range, ":company", 0, 8),
		...
		(team_set_slot, "$g_enemy_team", slot_team_pai_timer, 15),
		(try_end),
		(try_begin),
		...
		(try_end),
		(try_begin),
		(le, ":distance", 3000), # Entire formation melee charge at 30 meters distance
			(try_for_range, ":company", 0, 8),
			(call_script, "script_cf_if_company_has_soldiers", "$g_enemy_team", ":company"),
			(store_add, ":slot_team_pai_state", slot_team_company1_pai_state, ":company"),
			(team_slot_eq, "$g_enemy_team", ":slot_team_pai_state", company_pai_state_generic),
