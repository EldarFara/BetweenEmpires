...
VS_OUTPUT_TOON vs_main_standart_toon (uniform const int PcfMode, uniform const bool use_bumpmap, uniform const bool use_skinning, 
										float4 vPosition : POSITION, float3 vNormal : NORMAL, float2 tc : TEXCOORD0,  float3 vTangent : TANGENT, float3 vBinormal : BINORMAL, 
										float4 vVertexColor : COLOR0, float4 vBlendWeights : BLENDWEIGHT, float4 vBlendIndices : BLENDINDICES)
{
	INITIALIZE_OUTPUT(VS_OUTPUT_TOON, Out);
...
{ 
    ...
    fresnel = smoothstep(0.75, 0.77, fresnel);
    ...
    float4 light = saturate(ambientColor/2 + ambientSun/2 + ambientSky/2 + (sunIntensity * isLit * vSunColor));
   
    light -= shadow * 0.25;

    Output.RGBColor = color * In.VertexColor;
    Output.RGBColor *= light;
    Output.RGBColor += (specular * vSunColor * isLit);
    Output.RGBColor *= 1 - fresnel;   
	return Output;
}
...
technique toon
{
	pass P0
	{
		VertexShader = compile vs_2_0 vs_main_standart_toon(PCF_DEFAULT, false, true);
