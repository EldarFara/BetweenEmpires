	...
	try:
		...
		for index in xrange(len(source)):
			...
			if (name in uid[0]) and (name not in uid[1]):
				if WRECK.suppress_duplicate_warnings: # changed from if not WRECK.suppress_duplicate_warnings: to not display this
...
def parse_module_code(code_block, script_name, check_can_fail = False):
	...
	for unused_local in locals_def - locals_use:
		if not unused_local.startswith("unused"):
			WRECK.notices.append('local variable :%s declared but never used in %s' % (unused_local, script_name))
